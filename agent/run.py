...
class WebSocketManager:
    ...
    async def start_streaming(self, task, report_type, agent, websocket, api_key, email):
        report, path = await run_agent(task, report_type, agent, websocket, api_key, email)
        return report, path


async def run_agent(task, report_type, agent, websocket, api_key, email):
    #openai.api_key = api_key

    start_time = datetime.now()
    print(f"Start time: {start_time}")
    document_id = query2db(task, agent, report_type, start_time, email)
